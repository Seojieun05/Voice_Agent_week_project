...
QUESTION_GUIDANCE: Mapping[str, str] = {
    ...
    "can_i_go": (
        ...
        "기본 답은 '네'이고, 판단이 애매할 때도 '네'입니다. 다음 큰 문제가 있을 때만 "
        "'아니요'라고 합니다: 전방(중앙 또는 전방 전체)에 distance가 '가까움'인 "
        "장애물이 있음, 사람이 아주 많아 지나갈 틈이 없음, 차·오토바이·자전거·킥보드가 "
        "빠르게 다가오고 있음, 인도가 아니거나 진행할 길이 없음. "
        "distance가 '중간'이나 '멀리'인 물체, 왼쪽/오른쪽에만 있는 물체는 '아니요'의 근거가 "
        "아닙니다. 가깝지 않은 사람이 2~3명 정도 있는 것은 전혀 문제가 되지 않으므로 "
        "'네'라고 합니다. 이미지가 첨부되어 있으면 이미지로 길 상태와 거리감을 직접 "
        "판단합니다. '네'라고 답할 때 주의할 물체가 있으면 '다만 ~에 있는 ~는 주의하세요'를 "
        "덧붙입니다. "
