...
로컬 MP4에서는 **YOLO26s + ByteTrack**, 실시간 서버에서는 기본 **BoT-SORT**로 객체를
탐지·추적합니다. 한 신호등의 바운딩박스를 안정적으로 선택한 뒤 crop 기반의 실험적 HSV
분류기로 `RED`, `GREEN`, `YELLOW`, `UNKNOWN`을 판정하고, 연속된 결과가 충분히 쌓였을
때만 상태 전환 이벤트를 기록합니다.
...
```text
VisionSession → YOLO26 탐지 → tracker/stable ID → ObjectRouter
...
→ AnalysisResult → SceneEventManager → NarrationPolicy → NarrationScheduler
```
실제 배치 흐름은 다음과 같습니다.
...
이번 저장소는 서버가 narration 문자열을 반환하는 단계까지 구현합니다. 스마트폰 앱과 TTS
재생은 아직 구현하지 않습니다.
...
신호등의 HSV confidence는 보정된 확률이 아니라 색상 근거 점수입니다. JSONL attributes에는
`detection_confidence`와 `signal_evidence_confidence`를 따로 남기며, 상태 이벤트의 최종
confidence는 두 값의 최솟값입니다. 낮은 confidence 전환은 이벤트로 기록돼도 발화되지 않습니다.

일반 객체의 `OBJECT_APPEARED`와 `OBJECT_DISAPPEARED`도 JSONL에는 유지하지만 기본 발화
목록에서는 제외합니다. `--narrate-presence-classes`로 명시한 클래스만 다시 허용할 수 있으며,
이 설정은 YOLO의 `--classes` 탐지 필터와 별개입니다.
...
연결 직후 첫 text 메시지는 start JSON이어야 합니다.
...
각 프레임은 frame JSON과 JPEG binary를 바로 이어서 보냅니다.
...
성공 응답은 원본 `sequence_id`, 누적 drop 수, 구조화 이벤트, 발화 문자열과 서버 지연을
포함합니다.
...
```json
{
  ...
  "sequence_id": 101,
  "captured_at_ms": 1780000000000,
  "server_received_at_ms": 1780000000050,
  "completed_at_ms": 1780000000140,
  "dropped_frames": 3,
  "received_frames": 12,
  "processed_frames": 9,
  "processing_fps": 11.7,
  ...
  "timings": {
    ...
    "inference_ms": 70.2,
    "analysis_ms": 5.7,
    "total_server_ms": 81.4
  }
}
```

잘못된 JPEG, 크기 초과, rate limit, 메시지 순서 오류는 `type=error`와 명시적인 `code`로
...
```bash
python scripts/stream_video_client.py \
  --source samples/visiontest2.mp4 \
  --url ws://127.0.0.1:8000/ws/vision
```

동시 활성 WebSocket은 하나만 허용하며 두 번째 연결은 `SESSION_BUSY`로 거부됩니다. 연결
종료 시 tracker, stable ID, Analyzer, event history, narration queue를 모두 초기화합니다.
수신 JPEG와 영상은 기본적으로 저장하지 않고 binary/base64도 로그에 남기지 않습니다.
디버그 저장이 꼭 필요할 때만 `VISION_SERVER_DEBUG_FRAME_DIR`을 명시합니다.

로컬 MP4 기본 tracker는 하위 호환성을 위해 `bytetrack.yaml`입니다. live 기본
`botsort.yaml`은 설치된 Ultralytics 설정의 `sparseOptFlow` global motion compensation을
사용하지만 실제 착용 환경에서 항상 더 낫다고 보장하지 않습니다.
`VISION_SERVER_TRACKER=bytetrack.yaml`로 바꿀 수 있습니다. 버스 접근은 bbox 크기 변화에
의존해 카메라 자체 이동을 접근으로 오인할 수 있으므로 live 발화는 기본 비활성화됩니다.
이벤트와 분석 결과는 계속 반환되며, 검증 후에만
...
--kiosk-ocr-interval-frames    키오스크 OCR 호출 간격, MP4 기본 1
--text-ocr-interval-frames     표지판·화면 OCR 호출 간격, MP4 기본 1
...
--narrate-presence-classes     등장·사라짐 발화를 허용할 클래스 목록, 기본 없음
--no-bus-approach-narration    버스 접근 이벤트 발화 비활성화
```
...
- 서버의 실시간 JPEG 입력과 narration text 반환까지만 구현했습니다. 스마트폰 앱과 TTS
  재생은 아직 구현하지 않았습니다.
